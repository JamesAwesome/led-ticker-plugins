...
One wheel, five plugin namespaces: four sprite-trail families (`nyancat`, `pokeball`, `pacman`, `sailor_moon`) plus `flair` (the wheel's own name, for text effects). Each sprite family lives under `src/led_ticker_flair/<family>/`, registered in `src/led_ticker_flair/<family>/__init__.py` as `forward` / `reverse` / `alternating`. The `flair` namespace registers text effects: one animation (`flair.propeller`) and one transition (`flair.spinout`).
...
- **`flair` namespace naming exception** — unlike the four sprite families, the `flair` namespace is named after the wheel itself, not a sprite character. This is an intentional exception to the namespace-per-sprite-family pattern. It registers text effects — the `flair.propeller` animation and the `flair.spinout` transition — and has no `sprites/` directory.
