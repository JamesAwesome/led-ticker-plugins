...
# Shorten only on a paid tier. Delayed data (~1–15 min) means faster polling
# shows nothing new anyway.
update_interval = 360
...
- **Credit budget:** the free tier is ~800 credits/day (1 credit per symbol per poll). Unlike the Finnhub path, Twelve Data symbols are **not** frozen while their market is closed — each polls every cycle — so pick `update_interval` (widget) / `interval` (source) to stay in budget: `interval ≥ 108 × symbol_count` seconds keeps you under 800/day. Example: 3 symbols → `interval = 360` (720/day).
- **Also update the pre-existing "Equities only" section** of the README: the "FX requires a paid tier / not on the roadmap" note is now stale — forex/crypto ship via `provider = "twelvedata"` as of v0.6.0. Reconcile it: keep the fact that **Finnhub's free tier** is equities-only, but point readers to the new multi-asset section for FX/crypto rather than saying it's out of scope.
